...
GRUBuilder::GRUBuilder(unsigned layers,
  ...
  for (unsigned i = 0; i < layers; ++i) {
    // z
    Parameter p_x2z = local_model.add_parameters({hidden_dim, layer_input_dim}, scale);
    Parameter p_h2z = local_model.add_parameters({hidden_dim, hidden_dim}, scale);
    Parameter p_bz = local_model.add_parameters({hidden_dim}, scale);

    // r
    Parameter p_x2r = local_model.add_parameters({hidden_dim, layer_input_dim}, scale);
    Parameter p_h2r = local_model.add_parameters({hidden_dim, hidden_dim}, scale);
    Parameter p_br = local_model.add_parameters({hidden_dim}, scale);

    // h
    Parameter p_x2h = local_model.add_parameters({hidden_dim, layer_input_dim}, scale);
    Parameter p_h2h = local_model.add_parameters({hidden_dim, hidden_dim}, scale);
    Parameter p_bh = local_model.add_parameters({hidden_dim}, scale);
